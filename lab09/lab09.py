...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        tmp = subseqs(s[1:])
        return tmp + insert_into_all(s[0], tmp)
...
def non_decrease_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            return insert_into_all(s[0], ______________) + ________________
    return subseq_helper(s, ____)

def non_decrease_subseqs_v2(s):
    # get [0, idx) non-decreasing subsequences 
    def recur(idx):
        if idx < 0:
            return [[]]
        tmp = recur(idx-1)
        ret = tmp[:]  # avoid dead loop
        for t in tmp:
            if len(t) > 0 and t[len(t)-1] > s[idx]:
                continue
            ret.append(t + [s[idx]])
        return ret
