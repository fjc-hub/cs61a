...
def perfect_square(score):
    return deconposeSqure(score) != 0


def next_perfect_square(d):
    d = deconposeSqure(d)
    return (d+1)**2


def deconposeSqure(x):
    i = 1
    while i <= x // i :
        i += 1
    i -= 1
...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal :  # differ from 'score0 < goal & score1 < goal'
        if who == 0 :
...
def always_roll(n):
    ...
    """
    assert n >= 0 and n <= 10
    def strategy(scores, opponent_score):
        return n
    return strategy
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    """
    tmp = tail_points(opponent_score)
    if threshold <= tmp :
        return 0
    return num_rolls
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    tmp = tail_strategy(score, opponent_score, threshold, num_rolls)
    if tmp == 0 :
        return 0
    next = next_perfect_square(tail_points(opponent_score) + score)
